function [Lam1, Lam2] = Lyche_c_2D_Periodic(F,G,space)
...
%             |  ...             ...
%
% dobbiamo allora usare Lyche_c_1D_Periodic lungo ogni colonna, così avremo dei 
% d.o.f. colonna per colonna! poi dobbiamo usare la proiezione Lyche_1D_Periodic
...
%per costruzione 
coeff1 = [lam1(:,end - space.degree(1)*2 : end) lam1(:,2:end) lam1(:,2:space.degree(1)*2+1)];
% ora che abbiamo tamto coefficienti facciamo proiezione nell'altro senso.
for i = 1 : size(lam1,1) % proiettiamo le righe
    Lam1(i,:) = Lyche_1D_Periodic(coeff1(i,:), space.sp_univ(1)); %chiaramente lungo y
end
...
for i = 1 : size(lam2,1) % proiettiamo le righe
    Lam2(i,:) = Lyche_c_1D_Periodic(lam2(i,:), space.sp_univ(1)); %chiaramente lungo y
end
Lam2 = reshape(Lam2', space.ndof, 1); % CONTROLLA che il reshape sia giusto.
end