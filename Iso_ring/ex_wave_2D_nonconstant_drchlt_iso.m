...
filename     = '2D_dirichlet_u0_128x128_degree_6x6_eig4_sine_c_iso.mat'; % c=2;
...
exact_p1  = @(t) c(XP,YP).*squeeze(G_U0(1,:,:))*real(exp(1i*omega*(t))); %p1
exact_p2  = @(t) c(XP,YP).*squeeze(G_U0(2,:,:))*real(exp(1i*omega*(t))); %p2
[U0  , ~] = sp_eval (u_0, space_H, geometry_H, vtk_pts);
exact_v = @(t) U0*real(1i*omega*exp(1i*omega*(t))); %v
% Plot at t = 0.
figure ('Units', 'pixels', 'Position', [100 200 1000 350]) ;
subplot (1,2,1)
