...
%
%   errl2 = my_l2_error (space, msh, u, uex)
%
% INPUT:
%
...
function errl2 = my_l2_error (space, msh, u, uex, spex, coeff)
  ...
  if (numel(u) ~= space.ndof)
    error ('Wrong size of the vector of degrees of freedom for u')
  end
  
  if (numel(uex) ~= spex.ndof)
    error ('Wrong size of the vector of degrees of freedom for uex')
  end
