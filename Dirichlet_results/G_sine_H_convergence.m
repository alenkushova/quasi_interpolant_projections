...
for i = 1 : 4
    N = 2^(i+2); h = [h 1/N];
    % load the workspace
    s = ['Galerkin\sine\test_c_sine_'...
            num2str(N) 'x' num2str(N) 'x2000_drchlt.mat'];    load(s);
...
figure
subplot(1,2,1)
loglog(h,ERR_INF(1,:),'-*',h,ERR_INF(2,:),'-*',h,h.^3*100,'Linewidth', 1.5)
title('h-convergence in 2D')
grid on
legend('|| p-p_h ||_{\infty}', '|| v-v_h ||_{\infty}',...
    'h^3','Location','northwest')
subplot(1,2,2)
loglog(h,ERR_L2(1,:),'-*',h,ERR_L2(2,:),'-*',h,h.^3*4,'Linewidth', 1.5)
title('h-convergence in 2D')
grid on
