...
filename     = '2D_dirichlet_u0_128x128_degree_6x6_eig4_sine_c.mat'; % c=2;
...
method_data.T            = 30;      % Final time (Time step * Numero di elementi in tempo)
method_data.k            = 1.e-3;  %method_data.nsub(1); % Time step così varia solo lui.
...
%errore in spazio
for n = 1 : 200 : round(T/k)+1 %step "1:200:round(T/k)+1" per h conv.
...
filename = ['test_c_sine_energy_' num2str(n) 'x' num2str(n) 'x' num2str(T/k) '_drchlt.mat'];
save(filename)
