...
%  problem_data.c_diff         = @(x,y) sin(2*pi*x+2*pi*y);
%  problem_data.grad_c_diff    = @(x, y) cat (1, ...        % \nabla c
%                        reshape (2*pi*cos(2*pi*x+2*pi*y), [1, size(x)]), ...
%                        reshape (2*pi*cos(2*pi*x+2*pi*y), [1, size(x)]));                   
...
problem_data.periodic_directions = [2];         % periodic directions 
...
tic 
% 3) CALL TO SOLVER, dof of overkilled solution:
...
filename = ['2D_mixed_u0_', num2str(n1),'x',num2str(n2),'_degree_',...
    num2str(d1),'x',num2str(d2),'_eig4__iso.mat'];
