...
for i = 1 : 4
    ...
    s = ['Projections\sine\test_c_sine_'...
            num2str(N) 'x' num2str(N) 'x' num2str(N) '_drchlt.mat'];    load(s);    
...
figure
subplot(1,2,1)
loglog(h,ERR_INF(1,:),'-*',h,ERR_INF(2,:),'-*',h,h.^2*1600,'Linewidth', 1.5)
title('k-convergence in 2D')
grid on;
legend('|| p-p_h ||_{\infty}', '|| v-v_h ||_{\infty}',...
    'h^2','Location','northwest')

subplot(1,2,2)
loglog(h,ERR_L2(1,:),'-*',h,ERR_L2(2,:),'-*',h,h.^2*700,'Linewidth', 1.5)
title('k-convergence in 2D')
grid on; 
legend('|| p-p_h ||_2', '|| v-v_h ||_2', 'h^2',...
    'Location','northwest')
