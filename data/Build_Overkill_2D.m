% This scripts builds an overkill solution for the generalized eigenvalue
% problem (Helmoltz equation with non constant coefficient) 
% It stores the data in the a file useful for being uploaded in the solver
...
% problem_data.c_diff         = @(x, y) sin(2*pi*x)+2;
% problem_data.grad_c_diff    = @(x, y) cat (1, ...        % \nabla c
%                       reshape (2*pi*cos (2*pi*x), [1, size(x)]), ...
...
method_data.BC_H         = [5 5];     % Continuity at the boundaries (max = degree-1) 

% dof of overkilled solution:
tic
[geometry_H, msh_H, space_H, u_0, omega] = ...
...
filename = ['u0_', num2str(n1),'x',num2str(n2),'_degree_',...
    num2str(d1),'x',num2str(d2),'.mat'];
save(filename, 'geometry_H', 'msh_H', 'space_H', 'u_0', 'omega');
