...
% [space_p, space_v] = sp_bspline_fluid ('RT', ...
%                 geometry.nurbs.knots, nsub, degree, regularity, msh);
% mi da gradi di libertà non compatibili con la proiezione, o meglio,
% altrimenti devo ristudiare la scrittura dei punti su cui proiettare.
...
% trova il modo di mappare la meshgrid sul dominio fisico! forse conviene
% prima mappare i pnt e pnt_m e poi fare la mshgrid. 

% load(['drchlt_Theta_' num2str(nsub(1)) '_sine_iso.mat']);
% load('drchlt_Theta_' num2str(nsub(1)) '_constant_iso.mat');
...
for i = 1 : space_p.ndof
    ...
    [theta1, theta2] = Lyche_c_2D_Drch_new (b1,b2, space);
