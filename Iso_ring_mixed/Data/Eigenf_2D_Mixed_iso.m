function [geometry_H, msh_H, space_H, u_0,...
    omega, space_u1, space_u2, Du0_x1, Du0_x2] =...
                        Eigenf_2D_Mixed_iso (problem_data, method_data)
...
[rknots_H, zeta_H] = kntrefine (geometry_H.nurbs.knots, nsub_H-1, degree_H, regularity_H);
...
D_space_H = sp_vector (scalar_spaces, msh_H, 'curl-preserving'); % space S{p,p}.
...
% CONTROLLARE NEL CASO ISOGEOMETRICO CHE SIANO SCRITTE CORRETTAMENTE!!!!
d1 = method_data.degree_H(1);  d2 = method_data.degree_H(2);
%g/(xi_{i+g+1}- \xi{i+1})
step_h1 = space_H.knots{1}(1+d1+1:end-1) - space_H.knots{1}(2:end-d1-1); 
%g/(xi_{i+g+1}- \xi{i+1})
alpha1 = d1./step_h1; %Dirichlet direction weight for derivatives
alpha2 = d2/(space_H.knots{2}(d2+2) - space_H.knots{2}(2));% peso der. dir. 2 periodic
