...
function [errl2, errl2_elem] = my_l2_error (sp, msh, u, uex, spex, coeff)
  
  valu = sp_eval_msh (u, sp, msh);
  valu = reshape (valu, sp.ncomp, msh.nqn, msh.nel);
  
%  valex = sp_eval_msh (uex, spex, msh).*coeff(msh.qn{:});
  valex = sp_eval_msh (uex, spex, msh).*coeff;% PER 2D: .*coeff(msh.quad_nodes(1,:,:), msh.quad_nodes(2,:,:));
  valex = reshape (valex, spex.ncomp, msh.nqn, msh.nel);
