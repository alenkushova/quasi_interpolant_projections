function [Lam1, Lam2] = Lyche_c_2D_Drch_new (F,G,space)
...
if isa(F,'function_handle') && isa(G,'function_handle')
    ...
    pnt      = linspace (0, 1, 2*N +1); % vertici e punti medi
    pnt_m    = linspace (0, 1, 4*N +1); % ulteriori punti medi per integrazione numerica
...
for i = 1 : size(f1,2) % proiettiamo le colonne 
    lam1(:,i) = Lyche_c_1D_new(f1(:,i), space.sp_univ(2)); 
end
%per costruzione 
